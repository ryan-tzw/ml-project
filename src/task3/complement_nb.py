"""Task 3 ComplementNB search workflow (stage1 halving + stage2 finalist CV)."""
...
STAGE1_OUTPUT_PATH = OUTPUT_DIR / "complement_nb_halving_results.csv"
STAGE2_OUTPUT_PATH = OUTPUT_DIR / "complement_nb_finalists_cv5.csv"
SUBMISSION_PATH = OUTPUT_DIR / "ComplementNB_Prediction.csv"
...
def build_pipeline() -> Pipeline:  # Build TF-IDF + ComplementNB pipeline.
    return Pipeline(
        steps=[
            ...
            (
                "clf",
                ComplementNB(
                    norm=False,  # tuned via param grid
                ),
            ),
        ]
    )
...
def _load_stage1_top_params(top_k: int) -> list[dict[str, Any]]:
    ...
    if df.empty:
        raise ValueError("Stage1 results CSV empty.")
    sort_col = (
    ...
    if not param_cols:
        raise ValueError("No param columns found in stage1 results")
...
def run_stage1_halving_grid() -> (
    pd.DataFrame
):  # Run coarse hyperparameter search w HalvingGridSearchCV
...
def run_stage2_cv5_finalists(
    top_k: int = STAGE2_TOP_K_DEFAULT,
) -> pd.DataFrame:  # Re-evaluate top-k stage1 candidates with 5-fold CV
    stage_start = time.perf_counter()
...
def run_predict() -> (
    None
):  # Train the best model on the full train set and generate test predictions
    print("Loading cleaned data for final prediction")
    train_df, test_df = load_train_test_cleaned_data(PROJECT_ROOT)
    X_train = train_df["cleaned_abstract"].to_numpy(dtype=str)
    y_train = train_df["label_id"].to_numpy(dtype=np.int64)
    X_test = test_df["cleaned_abstract"].to_numpy(dtype=str)

    # Pick best params, prefer stage2 results, fall back to stage1
    best_params: dict[str, Any] = {}
    if STAGE2_OUTPUT_PATH.exists():
        stage2_df = pd.read_csv(STAGE2_OUTPUT_PATH)
        if not stage2_df.empty:
            param_cols = [c for c in stage2_df.columns if c.startswith("param_")]
            row = stage2_df.iloc[0]
            best_params = {
                col.replace("param_", "", 1): _coerce_param_type(
                    col.replace("param_", "", 1), row[col]
                )
                for col in param_cols
                if not pd.isna(row[col])
            }
    else:
        best_params = _load_stage1_top_params(top_k=1)[0]

    print(f"Best params for final model: {best_params}")
    ...
    submission = pd.DataFrame({"id": test_df["id"], "label_id": y_pred})
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Task 3 ComplementNB search workflow with staged CV."
    )
    parser.add_argument(
        "--stage1",
        action="store_true",
    ...
    parser.add_argument(
        "--predict",
        action="store_true",
        help="Train best model on full train set and generate test predictions.",
    )
    parser.add_argument(
        "--top-k",
        type=int,
    ...
    if not args.stage1 and not args.stage2 and not args.predict:
        parser.error("Pass at least one flag: --stage1, --stage2, --predict")
    if args.top_k <= 0:
